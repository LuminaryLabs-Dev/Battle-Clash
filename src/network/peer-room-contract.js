...
export function createAuthenticatedHello({ roomId, userId, role, profileRevision = 0, reconnectToken = null } = {}) {
  ...
  return Object.freeze({ schema: ROOM_CONTRACT_SCHEMA, roomId, userId, role, profileRevision: Math.max(0, Number(profileRevision) || 0), reconnectToken });
}

export function validateAuthenticatedHello(hello, expectedRoomId) {
  const accepted = hello?.schema === ROOM_CONTRACT_SCHEMA && hello.roomId === expectedRoomId && Boolean(hello.userId) && ["attacker", "defender"].includes(hello.role);
...
export function createMatchReceipt({ roomId, authorityId, profileRevision = 0, sequenceStart, sequenceEnd, result, rewardIdempotencyKey } = {}) {
  if (!roomId || !authorityId || !result || !rewardIdempotencyKey || sequenceEnd < sequenceStart) throw new TypeError("invalid match receipt");
...
export function acceptMatchReceipt(receipt, { roomId, authorityId, lastAcceptedKey = null } = {}) {
  const accepted = receipt?.schema === "battle-clash.match-receipt/1" && receipt.roomId === roomId && receipt.authorityId === authorityId && receipt.rewardIdempotencyKey !== lastAcceptedKey && Number.isInteger(receipt.sequenceStart) && Number.isInteger(receipt.sequenceEnd) && receipt.sequenceEnd >= receipt.sequenceStart;
