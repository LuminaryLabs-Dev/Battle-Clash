...
export function createAuthenticatedHello({ roomId, userId, role, profileRevision = 0, reconnectToken = null } = {}) {
  if (!roomId || !userId || !USER_ID_PATTERN.test(String(userId)) || !["attacker", "defender"].includes(role)) throw new TypeError("room hello requires room, UUID user, and role");
...
export function validateAuthenticatedHello(hello, expectedRoomId, options = {}) {
  const accepted = hello?.schema === ROOM_CONTRACT_SCHEMA
    ...
    && USER_ID_PATTERN.test(String(hello.userId ?? ""))
    ...
    && Number.isInteger(hello.profileRevision)
    && hello.profileRevision >= 0
